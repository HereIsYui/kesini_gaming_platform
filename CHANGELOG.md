...
P5 第一阶段已完成：玩家端新增“好友”入口，登录后可查看好友、收到的申请和发出的申请。公开主页可向其他玩家发起好友申请并显示好友状态；好友页支持通过、拒绝、取消申请和删除好友。好友页和排行榜可跳转玩家公开主页。
...
鱼排 VIP 状态查询已按会员接口文档增强兼容。优先查询指定用户会员信息，直接结果未命中时会使用活跃会员配置列表兜底，并兼容过期时间字段别名和时间戳。

游戏 VIP 已统一为 VIP1-VIP4 四档。鱼排 `VIP1_*` 至 `VIP4_*` 会映射到对应等级；鱼排 `VIP3_*` 或“小冰VIP”按 VIP3 处理，鱼排 `VIP4_*` 或“小冰白金VIP”按 VIP4 处理，多来源取最高档。关卡扫荡改用统一游戏 VIP 等级，登录卡片展示游戏 VIP 并支持每日礼包领取。

运营台新增“VIP福利”配置，可维护每档每日扫荡次数、交易手续费减免和每日礼包。交易挂单创建时按卖家的当前游戏 VIP 锁定手续费减免；已存在挂单仍按挂单记录中的手续费率结算。

新增两档月卡：小冰月卡对齐 VIP3，白金月卡对齐 VIP4。玩家可在“月卡”页使用鱼排积分购买或续费，运营台可配置月卡开关、价格和查看购买记录；小冰VIP和小冰白金VIP视为永久月卡来源，并与鱼排VIP、月卡订阅一起取最高档。
...
CREATE TABLE vip_daily_claim (
  ...
  createdAt datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
  PRIMARY KEY (id),
  UNIQUE KEY IDX_vip_daily_claim_unique (uid, claim_date),
  KEY IDX_vip_daily_claim_uid_created (uid, createdAt)
);

CREATE TABLE monthly_card_subscription (
  id int NOT NULL AUTO_INCREMENT,
  uid varchar(255) NOT NULL,
  ...
  updatedAt datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  PRIMARY KEY (id),
  UNIQUE KEY IDX_monthly_card_subscription_unique (uid, card_type),
  KEY IDX_monthly_card_subscription_uid_expires (uid, expires_at)
);

CREATE TABLE monthly_card_purchase_record (
  id int NOT NULL AUTO_INCREMENT,
  uid varchar(255) NOT NULL,
  fishpi_user_name varchar(255) NOT NULL,
  card_type varchar(24) NOT NULL,
  vip_level int NOT NULL,
  request_id varchar(80) NOT NULL,
  fishpi_cost int NOT NULL,
  ...
  updatedAt datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  PRIMARY KEY (id),
  UNIQUE KEY IDX_monthly_card_purchase_request (uid, request_id),
  KEY IDX_monthly_card_purchase_uid_status (uid, status),
  KEY IDX_monthly_card_purchase_created (createdAt)
);
```

P8 第一阶段到第四阶段不需要生产 SQL。

`point_ledger_record.source_type` 是 `varchar` 字段，新增“消息奖励”和“VIP礼包”来源不需要额外 SQL。
