...
- 玩家前端能打开公告列表，查看详情并关闭顶部公告
- 运营台能创建玩家消息，玩家端能查看并标记已读
...
接口为 `GET /messages` 和 `POST /messages/:id/read`；运营接口为 `GET /admin/player-messages`、`GET /admin/player-messages/:id`、`POST /admin/player-messages`、`PATCH /admin/player-messages/:id` 和 `DELETE /admin/player-messages/:id`。
